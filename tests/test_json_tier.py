...
def _ok(raw: str) -> GateResponse:
    return GateResponse(raw_text=raw, status=OutputStatus.SUCCESS)


def test_cast_json_parses_into_json_response() -> None:
    """`.cast_json()` finalizes into a JsonGateResponse with `json_response` set (fences stripped)."""
    out = JsonRequestBuilder.model_construct(client=None)._finalize(_ok('```json\n{"city":"Paris","temp":20}\n```'))
    assert isinstance(out, JsonGateResponse)
    assert out.json_response == {"city": "Paris", "temp": 20}


def test_cast_extends_json_with_typed_value() -> None:
    """`.cast(T)` is a json request too: json_response AND the validated value, and IS-A JsonGateResponse."""
    out = CastedRequestBuilder.model_construct(client=None, model_type=Weather)._finalize(_ok('{"city":"Lyon","temp":18}'))
    assert isinstance(out, TypedGateResponse)
    assert isinstance(out, JsonGateResponse)
...
def test_server_json_object_is_preferred() -> None:
    ...
    resp = GateResponse(raw_text="", status=OutputStatus.SUCCESS, json_object={"server": True})
