...
def test_fake_call_returns_canned_reply() -> None:
    ...
    resp = asyncio.run(client.request(prompt="hi").call("m"))
...
def test_fake_callable_reply_sees_request() -> None:
    ...
    resp = asyncio.run(client.request(prompt="hi", operation="op").call("m"))
    assert resp.raw_text == "echo:op"


def test_fake_cast_json_parses_canned_json() -> None:
    client = FakeClient('{"a": 1}')
    resp = asyncio.run(client.request(prompt="hi").cast_json().call("m"))
...
def test_fake_call_stream_emits_words() -> None:
    ...
    async def drain() -> str:
        return "".join([c.text async for c in client.request(prompt="hi").call_stream("m")])
