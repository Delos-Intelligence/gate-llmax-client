...
def test_operation_flows_onto_media_request() -> None:
    ...
    builder = client.image("a cat", operation="studio.image", timeout=30)
...
def test_call_prefer_falls_back_sequentially(monkeypatch: Any) -> None:
    ...
    resp = asyncio.run(client.image("a cat", operation="test_call_prefer").call_prefer(["bad", "good"]))
...
def test_call_prefer_reraises_when_all_fail(monkeypatch: Any) -> None:
    ...
    with pytest.raises(LLMServerError):
        asyncio.run(client.image("a cat", operation="test_call_prefer").call_prefer(["m1", "m2"]))
